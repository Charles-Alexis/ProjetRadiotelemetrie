...
/** @brief
 *
 * This function is called when a multicast group listener registration was
 * successful, indicating that multicast packets coming from non-Thread IPv6
 * hosts can be sent to this device by the (primary) BBR.
 *
 * The off-mesh multicasts are sent to the Thread network in MPL packets, via
 * IPv6-in-IPv6 encapsulation.
 *
 * On a BBR, this handler can be used to configure the multicast groups on the
 * backbone link.  The application must use MLDv2 to communicate to the wider IPv6
 * LAN/WAN about the groups it needs to listen to.
 *
 * @param multicastGroupAddress   IPv6 address representing the multicast group
 */
...
/** @brief
 * This function is called when the stack knows about a new backbone prefix or if
 * a backbone server has become unavailable.
 *
 * "available" means we can configure a DUA address.
 *
 * @param prefix                  DUA prefix
 * @param prefixLengthInBits      length in bits of the prefix
 * @param available               whether we can configure a DUA address
 */
